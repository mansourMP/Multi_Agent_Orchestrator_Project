...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstream = await fetch(`${resolveControlPlaneBackendUrl()}/auth/access/password`, {
      method: 'POST',
      headers: {
    ...
    return Response.json({ detail: 'Control-plane password backup proxy is unavailable.' }, { status: 503 });
  }
}
