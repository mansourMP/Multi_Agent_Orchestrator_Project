...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    return await runtimeProxyResponse(`/connectors/vault/${normalized}/google-doc${suffix}`, {
      method: 'POST',
      body: rawBody || undefined,
      headers: rawBody ? { 'Content-Type': request.headers.get('content-type') || 'application/json' } : undefined,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Google Doc create failed.';
