...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    return await runtimeProxyResponse(`/setup/sessions/${normalized}/resume`, { method: 'POST' });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Setup session resume failed.';
