...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    const { status, payload } = await runtimeJsonRequest(`/runs/${encodeURIComponent(rawRunId)}/hard-kill`, {
      method: 'POST',
    ...
    const message = error instanceof Error ? error.message : 'Run hard kill proxy failed.';
