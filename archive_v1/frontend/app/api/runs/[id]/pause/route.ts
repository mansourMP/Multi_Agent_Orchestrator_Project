...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    const { status, payload } = await runtimeJsonRequest(`/runs/${encodeURIComponent(rawRunId)}/pause`, {
      method: 'POST',
    });
    return Response.json(payload, { status });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Run pause proxy failed.';
