...
8. Sage Agent Computer access modes are `Default`, `Custom`, and `Full Access`.
   `Custom` is editable Default-style guarded access, not Full Access.
   `Full Access` is intentionally broad Sage control of the selected Agent
   Computer after explicit warning consent; do not downgrade it into a partial
   mode without a new product decision and tests.
