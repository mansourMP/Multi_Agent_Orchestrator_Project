...
- Access mode: `Default`, `Custom`, `Full Access`

`Default` is guarded Agent Computer access. It runs low-risk work automatically
and asks before risky file, terminal, browser, message, payment, install, or
system actions.

`Custom` starts as Default-style guarded access and lets the owner edit grants.
Until a custom policy is saved, it behaves like Default, not Full Access.

`Full Access` means Sage can control the selected Agent Computer without
per-action Empyralis prompts, including broad filesystem, shell, browser data,
token, SSH key, secret, delete, and connected-account access. It does not bypass
owner binding, tenant/workspace isolation, revocation, offline/degraded state,
quota, billing, audit, stop/cancel, OS permissions, or the kill switch.

Full Access must stay Full Access. Do not make it a partial or safer-sounding
mode in code. The product control is explicit warning consent plus dedicated
Agent hardware recommendation.
