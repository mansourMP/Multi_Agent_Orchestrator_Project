...
| `/w/:workspaceId/channels` | Channel-related Sage surface or connector pane | Personal-channel/gateway calls are route-backed but not clearly unified in shell | `/api/personal-channels/gateways/{gateway_id}/channels`, WhatsApp/Telegram setup routes, generic personal-channel message routes | Gateway personal-channel state for Telegram, WhatsApp, Signal, iMessage, WeChat plus personal channel message state | Missing clear first-class shell component mapping for channels; likely needs explicit pane or tab link from Agent Computer status | Pair a gateway, open Channels, verify Telegram/WhatsApp/Signal/iMessage/WeChat state, send test message only after approval behavior is clear |
