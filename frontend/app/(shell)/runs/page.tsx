...
function workflowProgressSummary(meta?: RuntimeRunMeta | null): string {
  ...
  if (!counts) return `${meta.workflow_node_count} steps tracked.`;
  ...
  return ordered.length > 0
    ? `${ordered.join(' · ')}`
    : `${meta.workflow_node_count} steps tracked.`;
}
...
export default function ExecutionsPage() {
  ...
  const activeSummary = useMemo(() => {
    ...
    return {
      ...
      note: 'Sage is idle. Open Sage to start work or use the filters below to review past runs.',
    };
  }, [runSummary]);
    ...
    <div style={pageShellStyle()}>
      ...
      >
        <div style={{ display: 'grid', gap: DESIGN_TOKENS.space[3] }}>
          <div style={metaTextStyle()}>Operations</div>
        ...
        <div style={{ display: 'flex', gap: DESIGN_TOKENS.space[3], flexWrap: 'wrap', alignItems: 'center' }}>
          <Link
            href="/"
            style={mergeStyles(
      ...
      <section style={mergeStyles(panelStyle({ muted: true, padding: DESIGN_TOKENS.space[5] }), { display: 'grid', gap: DESIGN_TOKENS.space[4], background: DESIGN_TOKENS.color.surfaceMuted })}>
        <div style={{ display: 'grid', gap: DESIGN_TOKENS.space[2] }}>
          <div style={sectionTitleStyle()}>Filter runs</div>
          <div style={bodyTextStyle()}>Search by task or run ID, then narrow by state, agent, or tool.</div>
        </div>
      ...
      {loading ? (
        ...
        <section className="orion-empty">
          ...
          <div className="orion-empty-copy orion-empty-copy-spaced">
            {executions.length === 0
              ? 'Open Sage to start work, then review progress and outcomes here.'
