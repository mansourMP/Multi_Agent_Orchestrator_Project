...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return await runtimeProxyResponse(`/connectors/vault${suffix}`, { method: 'GET' });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Connector list proxy failed.';
...
export async function POST(request: NextRequest) {
  ...
  try {
    return await runtimeProxyResponse('/connectors/vault', {
      method: 'POST',
      body: rawBody || undefined,
      headers: rawBody ? { 'Content-Type': request.headers.get('content-type') || 'application/json' } : undefined,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Connector save failed.';
    return Response.json({ detail: message }, { status: 503 });
  }
}
