...
function importedCredentialMarker(item: VaultCredentialItem): boolean {
  ...
  return provider === 'openai' && (
    String(metadata.import_source || '').trim().toLowerCase() === 'codex_auth_file'
...
export async function POST(request: NextRequest) {
  ...
  const [credentialsResult, profilesResult] = await Promise.all([
    ...
    runtimeJsonRequest(`/providers/profiles?workspace_id=${encodeURIComponent(workspaceId)}&provider=openai`, { method: 'GET' }),
  ]);
  ...
  const credentialCreate = await runtimeJsonRequest('/credentials/vault', {
    ...
    body: JSON.stringify({
      ...
      mode: 'byok',
      metadata: importedOpenAiCredentialMetadata(),
      skip_validation: true,
      credentials: {
        oauth_token: snapshot.accessToken,
  ...
  if (credentialCreate.status >= 400) {
    return Response.json(
      credentialCreate.payload ?? { detail: 'Failed to import local OpenAI / Codex session.' },
  ...
  let attention = false;
  let message = 'OpenAI / Codex session imported from this Mac.';
  ...
  if (enableRuntime && credentialId) {
    ...
    if (!validationOk) {
      ...
      runtimeDetail = String(
        ...
        || 'The local OpenAI / Codex session was saved, but it is not ready for runtime use yet.',
      ).trim() || 'The local OpenAI / Codex session was saved, but it is not ready for runtime use yet.';
      message = 'OpenAI / Codex session imported, but it needs attention before runtime can use it.';
    } else {
    ...
    if (profileCreate.status >= 400) {
      ...
      message = 'OpenAI / Codex session imported, but runtime could not enable it yet.';
    } else {
