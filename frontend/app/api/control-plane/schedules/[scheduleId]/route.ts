...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  try {
    return await runtimeProxyResponse(`/schedules/${normalized}`, {
      method: 'PATCH',
...
export async function DELETE(_request: NextRequest, { params }: Params) {
  const rejection = enforceBffRouteGuard(_request, { methods: ['DELETE'] });
  if (rejection) return rejection;
  const authFailure = await requireControlPlaneSession(_request);
  if (authFailure) return authFailure;
  ...
  try {
    return await runtimeProxyResponse(`/schedules/${normalized}`, { method: 'DELETE' });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Schedule delete failed.';
    return Response.json({ detail: message }, { status: 503 });
  }
}
