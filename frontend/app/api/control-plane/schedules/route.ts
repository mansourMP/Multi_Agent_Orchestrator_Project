...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return await runtimeProxyResponse(`/schedules${suffix}`, { method: 'GET' });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Schedule proxy failed.';
...
export async function POST(request: NextRequest) {
  ...
  try {
    return await runtimeProxyResponse('/schedules', {
      method: 'POST',
