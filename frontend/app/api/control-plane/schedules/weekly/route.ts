...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return await runtimeProxyResponse(`/schedules/weekly${suffix}`, { method: 'GET' });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Weekly schedule proxy failed.';
...
export async function POST(request: NextRequest) {
  ...
  try {
    return await runtimeProxyResponse('/schedules/weekly', {
      method: 'POST',
      body: rawBody || undefined,
      headers: rawBody ? { 'Content-Type': request.headers.get('content-type') || 'application/json' } : undefined,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Weekly schedule create failed.';
    return Response.json({ detail: message }, { status: 503 });
  }
}
