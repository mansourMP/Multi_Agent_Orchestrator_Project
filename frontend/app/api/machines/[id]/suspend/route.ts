...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    const { status, payload } = await runtimeJsonRequest(`/machines/${machineId}/suspend`, {
      method: 'POST',
    ...
    const message = error instanceof Error ? error.message : 'Machine suspend proxy failed.';
