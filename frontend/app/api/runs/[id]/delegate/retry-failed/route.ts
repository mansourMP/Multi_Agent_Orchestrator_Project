...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    const { status, payload } = await runtimeJsonRequest(`/runs/${runId}/delegate/retry-failed`, {
      method: 'POST',
    ...
    const message = error instanceof Error ? error.message : 'Retry delegation proxy failed.';
