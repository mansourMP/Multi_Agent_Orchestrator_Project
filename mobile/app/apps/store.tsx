...
import type { AppRecord, MobileSession } from "@/src/lib/types";

function sessionHasRuntimeAccess(session: MobileSession | null) {
  return Boolean(session?.runtimeUrl && session?.runtimeKey);
}
  ...
  const handleAppAction = React.useCallback(
    async (app: AppRecord, installed: boolean, needsUpdate: boolean) => {
      if ((needsUpdate || !installed) && !canInstallOrUpdate(app)) {
        showBanner("Open the app to view it.", "neutral");
      ...
      if (!sessionHasRuntimeAccess(session)) {
        ...
        showBanner("This app is available in the beta store view.", "neutral");
        return;
      }
        ...
        >
          <View style={{ paddingHorizontal: 20, paddingTop: 18, paddingBottom: 10 }}>
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <TouchableOpacity
                activeOpacity={0.85}
                onPress={() => showBanner("Templates and apps ready for the current beta.", "neutral")}
                style={{
                  width: 34,
                  height: 34,
                  borderRadius: 17,
            ...
            {!loading ? (
              <>
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                  ...
                  <TouchableOpacity
                    activeOpacity={0.85}
                    onPress={() => showBanner("Showing the current beta app set.", "neutral")}
                  >
