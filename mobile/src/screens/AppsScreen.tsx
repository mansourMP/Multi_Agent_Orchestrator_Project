...
function appStatus(record: LauncherApp) {
  const status = readString(record.blueprint?.status);
  if (!status || status === "workspace_private") return "";
...
export default function AppsScreen() {
  ...
  const workspaceAppsQuery = useQuery({
    ...
    enabled: connected,
    queryFn: () => appRegistryApi.listWorkspaceApps(session!),
  });
  ...
  return (
    <MobileScreen>
      <PrimaryScreenHeader
        title="Apps"
        subtitle="Open the apps published or cloned into this workspace."
      />
      ...
      {loadError ? (
        ...
        >
          <Text style={{ color: theme.colors.textMuted, fontSize: theme.typography.meta, lineHeight: 20 }}>
            Apps are not available on this phone yet. Desktop and mobile use the same workspace launcher when this workspace is reachable.
          </Text>
        </View>
      ) : null}
      ...
      {workspaceAppsQuery.isLoading ? (
        ...
        <LauncherEmptyState
          title="No apps yet"
          detail="Publish a blank app on desktop or clone one from Discovery."
          icon="cube-outline"
        />
      )}
    </MobileScreen>
