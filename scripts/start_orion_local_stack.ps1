...
$TelegramAutopilotAllowAnyChat = if ($env:EMPYRALIS_TELEGRAM_AUTOPILOT_ALLOW_ANY_CHAT) { $env:EMPYRALIS_TELEGRAM_AUTOPILOT_ALLOW_ANY_CHAT } elseif ($env:ORION_TELEGRAM_AUTOPILOT_ALLOW_ANY_CHAT) { $env:ORION_TELEGRAM_AUTOPILOT_ALLOW_ANY_CHAT } else { "1" }
...
Write-Host "  curl -s -H `"X-API-Key: $cleanRuntimeKey`" `"$runtimeUrl/local/workers/status`""
