...
class TelegramMenuService:
    ...
    def prefixed_command(self, profile: Dict[str, Any], command_text: str) -> str:
    ...
    def menu_keyboard(self, profile: Dict[str, Any], menu_id: str = "main") -> Dict[str, Any]:
        ...
        status_cmd = self.prefixed_command(profile, "status") if require_prefix else "Status"
        approvals_cmd = self.prefixed_command(profile, "approvals") if require_prefix else "Approvals"
        help_cmd = self.prefixed_command(profile, "help") if require_prefix else "Help"
        me_cmd = self.prefixed_command(profile, "me") if require_prefix else "My context"

        if menu_id == "study":
            if require_prefix:
                keyboard = [
                    [{"text": self.prefixed_command(profile, "run inbox triage")}, {"text": self.prefixed_command(profile, "run draft message")}],
                    [{"text": self.prefixed_command(profile, "run today priorities")}, {"text": self.prefixed_command(profile, "run task breakdown")}],
                    [{"text": self.prefixed_command(profile, "menu")}],
                ]
            else:
                keyboard = [
            ...
            if require_prefix:
                keyboard = [
                    [{"text": self.prefixed_command(profile, "run project update")}, {"text": self.prefixed_command(profile, "run next steps")}],
                    [{"text": self.prefixed_command(profile, "run meeting prep")}, {"text": self.prefixed_command(profile, "run write follow-up")}],
                    [{"text": self.prefixed_command(profile, "menu")}],
                ]
            else:
                keyboard = [
            ...
            if require_prefix:
                keyboard = [
                    [{"text": self.prefixed_command(profile, "me")}, {"text": self.prefixed_command(profile, "help")}],
                    [{"text": self.prefixed_command(profile, "menu")}],
                ]
            else:
                keyboard = [
            ...
            if require_prefix:
                ...
                for skill in active_skills:
                    row.append({"text": self.prefixed_command(profile, f"skill {skill.get('id')}")})
                    if len(row) >= 2:
                        keyboard.append(row)
                        row = []
                if row:
                    keyboard.append(row)
                keyboard.append([{"text": self.prefixed_command(profile, "menu")}])
            else:
                keyboard = []
                row = []
                for skill in active_skills:
            ...
            if require_prefix:
                keyboard = [
                    [{"text": self.prefixed_command(profile, "menu work")}, {"text": self.prefixed_command(profile, "menu project")}],
                    [{"text": self.prefixed_command(profile, "menu skills")}, {"text": self.prefixed_command(profile, "menu context")}],
                    [{"text": status_cmd}],
