...
router.add_api_route("/approvals/audit", history.get_approval_audit, methods=['GET'], dependencies=[Depends(require_admin_api_key)])
router.add_api_route("/approvals", history.list_pending_approvals, methods=['GET'], dependencies=[Depends(require_api_key)])
router.add_api_route("/audit", history.get_audit, methods=['GET'], dependencies=[Depends(require_admin_api_key)])
