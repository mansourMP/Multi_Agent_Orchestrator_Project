...
class GatewayPhase7RoutesTests(unittest.TestCase):
    ...
    def test_gateway_browser_attach_mode_reports_attach_required_and_attached_states(self) -> None:
        ...
        with self.client.websocket_connect(ws_path) as websocket:
            ...
            attach_response: dict = {}

            def _post_attach() -> None:
                attach_response["response"] = self.client.post(
                    f"/api/gateway/registrations/{gateway_id}/browser/sessions",
                    json={
                        "session_mode": "existing_session_attach",
            ...
            attach_thread = threading.Thread(target=_post_attach)
            attach_thread.start()
            attach_frame = websocket.receive_json()
            self.assertEqual(attach_frame["payload"]["capability_id"], "browser.session.start")
            self.assertEqual(
                attach_frame["payload"]["arguments"]["session_mode"],
                "existing_session_attach",
            )
            self.assertEqual(
                attach_frame["payload"]["arguments"]["attach_endpoint_url"],
                "http://127.0.0.1:9222",
            )
            self.assertEqual(
                attach_frame["payload"]["arguments"]["browser_metadata"]["browser_attach_endpoint_url"],
                "http://127.0.0.1:9222",
            )
            websocket.send_json(
                {
                    ...
                    "id": attach_frame["id"],
                    "ok": True,
                    "ts": "2026-04-22T16:00:02Z",
                    "payload": {
                        "request_id": attach_frame["id"],
                        "capability_id": "browser.session.start",
                        "run_id": "run-browser-attach-1",
                        "result": {
                            "browser_session": {
                                "browser_session_id": "gbsess-attach-1",
                                "status": "attached",
                                "execution_target": "local_gateway",
                                "session_profile": "gateway_default",
                                ...
                                "reviewed_approval_required": False,
                                "reviewed_approved": False,
                                "immutable_plan_hash": "plan-attach-1",
                                "execution_binding": {"cwd": "/tmp/project"},
                                "checkpoint": {
                                    "next_action_index": 0,
                                    "session_profile": "gateway_default",
                                ...
                                "snapshot": {
                                    ...
                                    "tabs": [{"id": 1, "url": "https://example.com/account"}],
                                    "accessibility_snapshot": {"role": "document"},
                                },
                                "metadata": {
                                    "browser_session_mode": "existing_session_attach",
                                    "browser_attach_state": "attached",
                                    "browser_attach_endpoint_url": "http://127.0.0.1:9222",
                                    "browser_resume_supported": True,
                                },
                            },
                            "status": "started",
            ...
            self.assertEqual(attach_response["response"].status_code, 200)
            self.assertEqual(attach_response["response"].json()["browser_session"]["status"], "attached")

            approval_response = self.client.post(
                f"/api/gateway/registrations/{gateway_id}/browser/sessions/gbsess-attach-1/actions",
                json={
                    "action": "click",
                    "action_args": {"selector": "#account"},
                    "run_id": "run-browser-attach-1",
                    "trace_id": "trace-browser-attach-1",
                },
            )
            ...
            def _resolve_attach_approval() -> None:
                resolve_response["response"] = self.client.post(
                    f"/api/gateway/registrations/{gateway_id}/approvals/{approval_id}/resolve",
                    json={"decision": "approved", "note": "Attach action approved", "timeout_seconds": 5},
                )

            resolve_thread = threading.Thread(target=_resolve_attach_approval)
            resolve_thread.start()
            action_frame = websocket.receive_json()
            self.assertEqual(action_frame["payload"]["capability_id"], "browser.session.action")
            self.assertEqual(
                action_frame["payload"]["arguments"]["session_mode"],
                "existing_session_attach",
            )
            self.assertEqual(
                action_frame["payload"]["arguments"]["attach_endpoint_url"],
                "http://127.0.0.1:9222",
            )
            self.assertEqual(
                action_frame["payload"]["arguments"]["browser_metadata"]["browser_session_mode"],
                "existing_session_attach",
            )
            websocket.send_json(
                {
                    ...
                    "payload": {
                        ...
                        "result": {
                            "browser_session": {
                                ...
                                "reviewed_approval_required": True,
                                "reviewed_approved": True,
                                "immutable_plan_hash": "plan-attach-2",
                                "execution_binding": {"cwd": "/tmp/project"},
                                "checkpoint": {
                                    "next_action_index": 1,
                                    "session_profile": "gateway_default",
            ...
            self.assertEqual(resolve_response["response"].status_code, 200)
            self.assertEqual(
                resolve_response["response"].json()["execution"]["browser_session"]["status"],
                "attached",
            )
            ...
            self.assertEqual(doctor_payload["browser"]["count"], 2)
            self.assertEqual(doctor_payload["browser"]["active_count"], 1)
            browser_attach_check = next(
                item for item in doctor_payload["checks"] if item["id"] == "browser_attach"
            )
            self.assertEqual(browser_attach_check["status"], "pass")
