...
class GatewayProtocolServiceToolInvokeRustGateTests(unittest.TestCase):
    def test_gateway_protocol_message_decision_accepts_tool_invoke(self) -> None:
        with patch(
            "server_modules.gateway_protocol_service.rust_runtime_kernel_client.run_runtime_kernel_enforced",
            return_value={
                "ok": True,
                "decision": "allow",
                "message_type": "tool_invoke",
        ...
        self.assertEqual(payload["message_type"], "tool.invoke")
        self.assertEqual(payload["tool_name"], "computer_control.click")
        self.assertEqual(decision["next_action"], "dispatch_tool_invoke")
    ...
    def test_tool_execute_protocol_route_accepts_dispatch_gateway_operation(self) -> None:
        with patch(
            "server_modules.gateway_protocol_service.rust_runtime_kernel_client.run_runtime_kernel_enforced",
            return_value={
                "ok": True,
            ...
            decision = gateway_protocol_service._enforce_gateway_tool_execute_protocol_route(
                gateway_id="gw-1",
                session_id="sess-1",
                workspace_id="ws-1",
                tenant_id="tenant-1",
        ...
        self.assertEqual(payload["capability_id"], "computer_control.click")
        self.assertEqual(decision["next_action"], "dispatch_gateway_operation")

    def test_dispatch_tool_invoke_blocks_wrong_rust_action_before_send_request(self) -> None:
        connection = SimpleNamespace(
            session_id="sess-1",
            scope={"workspace_id": "ws-1", "tenant_id": "tenant-1"},
            send_request=AsyncMock(side_effect=AssertionError("should not send tool.invoke")),
        )
        ...
        async def run_test() -> None:
            with (
                ...
                with self.assertRaises(gateway_protocol_service.GatewayProtocolRustGateError) as raised:
                    await gateway_protocol_service.dispatch_tool_invoke(
                        gateway_id="gw-1",
                        capability_id="computer_control.click",
                        arguments={"x": 1, "y": 2},
                        run_id="run-1",
    ...
    def test_dispatch_tool_invoke_blocks_wrong_gateway_protocol_action_before_send_request(self) -> None:
        connection = SimpleNamespace(
            session_id="sess-1",
            scope={"workspace_id": "ws-1", "tenant_id": "tenant-1"},
            send_request=AsyncMock(side_effect=AssertionError("should not send tool.invoke")),
    ...
    def test_dispatch_tool_invoke_blocks_wrong_quota_action_before_send_request(self) -> None:
        connection = SimpleNamespace(
            session_id="sess-1",
            scope={"workspace_id": "ws-1", "tenant_id": "tenant-1"},
            send_request=AsyncMock(side_effect=AssertionError("should not send tool.invoke")),
