...
class TelegramRunDispatchServiceTests(unittest.TestCase):
    ...
    def test_dispatch_run_action_edits_ack_message(self) -> None:
        service = self._make_service(include_meta=lambda: True)
        service.wait_for_terminal_status = lambda run_id, timeout_seconds=None, max_reply_chars=None: {  # type: ignore[method-assign]
            "status": "completed",
        ...
        self.assertIn("Thinking...\nrun_id: run-1", sent_messages[0][0][2])
        self.assertEqual(len(edited_messages), 1)
        self.assertIn("run_id: run-1", edited_messages[0][0][3])
